...
import {
  Box, Button, Chip, Container, Grid, LinearProgress, Paper,
...
const CAPTURE_MS = 800;      // how often to grab a frame
const JPEG_QUALITY = 0.8;    // dataURL quality
...
export default function ASL() {
  ...
  const [accuracy] = React.useState(98.5); // UI display only
  const [procLoad, setProcLoad] = React.useState(0);
  const [mem, setMem] = React.useState(0);
  const [errorText, setErrorText] = React.useState("");

  // timers / inflight control
  ...
  // Simulate perf bars (UI only)
  React.useEffect(() => {
    if (!live) return;
    perfIntervalRef.current = window.setInterval(() => {
      setProcLoad((v) => Math.min(100, Math.max(0, v + (Math.random() * 14 - 7))));
      setMem((v) => Math.min(100, Math.max(0, v + (Math.random() * 8 - 4))));
    }, 1200);
    return () => {
  ...
  React.useEffect(() => {
    ...
    const doCapture = async () => {
      ...
      // cancel previous in-flight request before a new one
      requestAbortRef.current?.abort();
      const controller = new AbortController();
      requestAbortRef.current = controller;

      try {
        const res = await fetch(`${API_BASE}/asl/predict-frame`, {
          method: "POST",
    ...
    // first immediate capture, then interval
    doCapture();
  ...
  // Pause when tab hidden (saves CPU/bandwidth)
  React.useEffect(() => {
    const onVis = () => {
      if (document.hidden && live) endSession();
    };
              ...
              <Stack spacing={1.2} alignItems="center">
                ...
                <Box
                  sx={{
                    ...
                    boxShadow: "0 20px 40px rgba(0,0,0,0.12), inset 0 1px 0 rgba(255,255,255,0.2)"
                  }}
                >
                  <Box
                    sx={{
                      ...
                      boxShadow: "inset 0 2px 30px rgba(0,0,0,0.35), 0 10px 30px rgba(0,0,0,0.2)"
                    }}
                  >
                    {/* chips */}
                    <Chip
                ...
                {/* Actions */}
                <Stack direction={{ xs: "column", sm: "row" }} spacing={2} justifyContent="center" sx={{ mt: 3, width: "100%" }}>
                  {live ? (
                    <Button
                      variant="contained"
                      onClick={endSession}
                      ...
                      sx={{
                        ...
                        boxShadow: "0 14px 30px rgba(56,189,248,.25)",
                        fontWeight: 700
                      }}
                    >
                      End Session
                  ...
                  <Button
                    ...
                    disabled={!live}
                    onClick={async () => {
                      // one-shot manual predict using current frame
                      const v = videoRef.current;
                      ...
                      try {
                        ...
                        const json = await res.json().catch(() => ({}));
                        if (!res.ok) setLabel(json?.detail ?? "No hand / multiple hands");
                        else setLabel(json?.label ?? "");
                        setErrorText("");
                      } catch (err: any) {
                        if (err?.name !== "AbortError") setErrorText("API error");
                      }
                    }}
                    fullWidth
                    sx={{
